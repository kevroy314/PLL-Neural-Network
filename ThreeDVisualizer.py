...
class ThreeDVisualizer:
    def __init__(self, width, height):
        ...
        self.x = np.linspace(-8, 8, width + 1).reshape(width + 1, 1)
        self.y = np.linspace(-8, 8, height + 1).reshape(1, height + 1)

        ## create a surface plot, tell it to use the 'heightColor' shader
        ## since this does not require normal vectors to render (thus we
        ## can set computeNormals=False to save time when the mesh updates)
        self.p = gl.GLSurfacePlotItem(x=self.x[:, 0], y=self.y[0, :], colors=np.ones((50, 50, 4), dtype=float),
                                      shader='shaded', computeNormals=False, smooth=False)
        self.p.shader()['colorMap'] = np.array([0.2, 2, 0.5, 0.2, 1, 1, 0.2, 0, 2])
        self.p.translate(10, 10, 0)
        self.w.addItem(self.p)

    def update(self, _r, _i):
        self.p.setData(z=_i, colors=_r)